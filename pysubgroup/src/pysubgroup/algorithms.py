...
class DFS:
    ...
    def search_internal(self, task, result, sg):
        ...
        """
        if hasattr(task.qf, "calculate_size_statistics"):
            size_stats = task.qf.calculate_size_statistics(
                sg, task.target, task.data
            )
            if not constraints_satisfied(
                task.constraints_monotone, sg, size_stats, task.data
            ):
                return
            statistics = task.qf.calculate_statistics(
                sg, task.target, task.data, size_stats
            )
            # Size-based prune already applied; skip duplicate task.constraint check.
            recheck_constraints = False
        else:
            statistics = task.qf.calculate_statistics(sg, task.target, task.data)
            if not constraints_satisfied(
                task.constraints_monotone, sg, statistics, task.data
        ...
        ps.add_if_required(
            ...
            task,
            statistics=statistics,
            check_constraints=recheck_constraints,
        )
