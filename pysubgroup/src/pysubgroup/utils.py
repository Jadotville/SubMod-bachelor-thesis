...
def minimum_required_quality(result, task):
    ...
    Returns:
        ...
        set.
    """
    # result_set_size <= 0 means keep all subgroups (no top-k cutoff)
    if task.result_set_size <= 0 or len(result) < task.result_set_size:
...
def prepare_subgroup_discovery_result(result, task):
    ...
    Returns:
        ...
        ``result_set_size <= 0`` keeps all subgroups that pass ``min_quality``.
    """
...
def add_if_required(
    ...
    Parameters:
        ...
        check_constraints (bool): If True (default), re-check ``task.constraints``.
            Set False when the caller already applied the relevant constraints
            (e.g. monotone MinSupport before a local-model fit).
    ...
    if quality >= task.min_quality:
        if check_constraints and not ps.constraints_satisfied(
            task.constraints, sg, statistics, task.data
        ):
            return
        if check_for_duplicates and (quality, sg, statistics) in result:
            return
        # result_set_size <= 0 means keep all subgroups
        if explicit_result_set_size <= 0 or len(result) < explicit_result_set_size:
