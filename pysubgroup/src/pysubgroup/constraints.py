...
class MinUniqueValuesConstraint:
    ...
    def is_satisfied(self, subgroup, statistics=None, data=None):
        ...
        if hasattr(subgroup, "representation"):
            labels = data.loc[subgroup.representation, self.attribute_name]
        elif hasattr(subgroup, "covers"):
            labels = data[self.attribute_name].iloc[subgroup.covers(data)]
        else:
            cover, _ = ps.get_cover_array_and_size(subgroup, len(data), data)
            labels = data[self.attribute_name].iloc[cover]
        return labels.nunique() >= self.min_unique_values
