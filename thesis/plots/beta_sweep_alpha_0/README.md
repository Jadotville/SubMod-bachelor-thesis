...
`Q_gew = quality_raw · cb(S)^β`, skaliert die Qualität je Datensatz und
berechnet die Spearman-Rangkorrelation zwischen `cb(S)` und `Q_gew` je Modell.

Vom Repository-Root aus ausführen:

```bash
.venv/bin/python thesis/plots/beta_sweep_alpha_0/generate_plot.py
```
